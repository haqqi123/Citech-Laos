...
const LoginModal = ({ isOpen, onClose, onLogin }) => {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="bg-white rounded-[2rem] w-full max-w-md overflow-hidden shadow-2xl relative z-10 border border-slate-100">
...
const DashboardView = ({ reports }) => {
  ...
  const barData = {
    labels: ['Dilaporkan', 'Diproses', 'Selesai'],
    datasets: [{
      label: 'Jumlah Laporan',
      data: [statusCount['Dilaporkan'], statusCount['Diproses'], statusCount['Selesai']],
  ...
  return (
    <div className="pt-8 pb-20 bg-slate-50 min-h-screen px-4">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-extrabold mb-8 text-slate-900">Dashboard Analytics</h1>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm"><p className="text-sm font-bold text-slate-400 mb-1">Total Laporan</p><p className="text-4xl font-extrabold text-slate-900">{reports.length}</p></div>
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm"><p className="text-sm font-bold text-slate-400 mb-1">Menunggu</p><p className="text-4xl font-extrabold text-red-500">{statusCount['Dilaporkan']}</p></div>
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm"><p className="text-sm font-bold text-slate-400 mb-1">Diproses</p><p className="text-4xl font-extrabold text-amber-500">{statusCount['Diproses']}</p></div>
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm"><p className="text-sm font-bold text-slate-400 mb-1">Selesai</p><p className="text-4xl font-extrabold text-green-500">{statusCount['Selesai']}</p></div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-6 text-lg">Distribusi Kategori</h3>
            <div className="h-64 flex justify-center"><Pie data={pieData} options={{ maintainAspectRatio: false }} /></div>
          </div>
          <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
            <h3 className="font-bold text-slate-900 mb-6 text-lg">Status Penanganan</h3>
...
const MapPage = ({ reports, onVote, onDetail, onLocationSelected, pendingCoords, hideSidebar = false }) => {
  ...
  const getMarkerIcon = (r) => L.divIcon({html: `<div style="background-color: ${r.status==='Dilaporkan'?'#ef4444':r.status==='Diproses'?'#f59e0b':'#10b981'}; width: 30px; height: 30px; border-radius: 50%; border: 3px solid white; box-shadow: 0 4px 6px rgba(0,0,0,0.1);"></div>`, className: '', iconSize: [30,30]});
  const Picker = () => { useMapEvents({click(e) { if(onLocationSelected) onLocationSelected([e.latlng.lat, e.latlng.lng]); }}); return null; };
  
  return (
    <div className={`${hideSidebar ? 'h-[calc(100vh-6rem)] rounded-3xl overflow-hidden shadow-sm border border-slate-100' : 'pt-20 h-screen'} w-full relative flex`}>
      {!hideSidebar && (
        <div className="w-96 bg-white border-r flex flex-col h-full z-10 shadow-lg">
          <div className="p-6 border-b border-slate-100">
            <h3 className="font-extrabold text-xl text-slate-900 mb-4 flex items-center gap-2"><Filter className="h-5 w-5 text-brand-blue"/> Filter Peta</h3>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Kategori</label>
                <select className="w-full border border-slate-200 p-2.5 rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-brand-blue/20" value={filterCat} onChange={(e)=>setFilterCat(e.target.value)}>
              ...
              <div>
                ...
                <select className="w-full border border-slate-200 p-2.5 rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-brand-blue/20" value={filterStatus} onChange={(e)=>setFilterStatus(e.target.value)}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50">
            {filtered.map(r => (
              <div key={r.id} onClick={() => setMapCenter(r.coords)} className="bg-white p-4 rounded-2xl border border-slate-100 cursor-pointer hover:border-brand-blue hover:shadow-md transition-all">
                <div className="flex justify-between items-start mb-2">
                  <span className="text-xs font-bold text-brand-blue bg-blue-50 px-2 py-1 rounded-md">{r.category}</span>
                  <span className={`w-2.5 h-2.5 rounded-full ${r.status==='Dilaporkan'?'bg-red-500':r.status==='Diproses'?'bg-amber-500':'bg-green-500'}`}></span>
                </div>
                <h4 className="font-bold text-sm text-slate-900 line-clamp-2 mb-1">{r.title}</h4>
                <p className="text-xs text-slate-500 flex items-center gap-1"><MapPin className="h-3 w-3"/> {r.location}</p>
              </div>
            ))}
            {filtered.length === 0 && <p className="text-center text-sm text-slate-400 mt-10">Tidak ada laporan sesuai filter.</p>}
          </div>
        </div>
      )}
            ...
            <Marker key={r.id} position={r.coords} icon={getMarkerIcon(r)}>
              <Popup className="custom-popup">
                <div className="text-center p-1">
                  ...
                  <button onClick={() => onDetail && onDetail(r)} className="w-full bg-brand-blue text-white text-xs font-bold py-2 rounded-lg">Lihat Detail</button>
                </div>
              </Popup>
            </Marker>
          ))}
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <select className="border border-slate-200 rounded-xl px-4 py-2.5 outline-none font-bold text-slate-700 bg-white shadow-sm" value={filter} onChange={(e)=>setFilter(e.target.value)}>
    ...
    <div className="min-h-screen flex flex-col font-sans bg-slate-50">
      <nav className="fixed top-0 w-full z-50 glass shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-20 flex justify-between items-center">
          ...
          {user ? (
             <div className="flex items-center gap-3"><span className="text-sm font-bold">{user.email}</span><button onClick={handleLogout} className="text-red-500 font-bold text-sm">Logout</button></div>
          ) : (
      ...
      <main className="flex-1 flex flex-col">
        ...
        {currentView === 'dashboard' && <div className="pt-20"><DashboardView reports={reports} /></div>}
      </main>
